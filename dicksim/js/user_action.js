...
const makeSpringMenu = s => {
    ...
    const l_range = makeRangeInput('lenRange', 0.0, 2, 0.001, s.rst);
    const c_sel = makeOptionInput('colGroupSel', [
        { name: 'Default', val: 0 },
        { name: 'Universal', val: -1 },
        { name: '1', val: 1 },
    ...
    const l_setting = makeContainer('rangeContainer', l_range);

    s_range.oninput = () => s.stf = Number(s_range.value);
    d_range.oninput = () => s.dmp = Number(d_range.value);
    l_range.oninput = () => s.rst = Number(l_range.value);
    ...
    innerDiv.appendChild(makeInputLabel('lenRange', 'Length'));
    innerDiv.appendChild(l_setting);
    innerDiv.appendChild(makeInputLabel('colGroupSel', 'Collision Group:'));
    innerDiv.appendChild(c_sel);
...
ctrlButtons['rstBtn'].addEventListener('click', e => {
    if (window.confirm('You are about to reset the model. Continue?'))
        Model.dickNBalls();
}, true);
ctrlButtons['saveBtn'].addEventListener('click', e => {
    Model.save();
}, true);
ctrlButtons['loadBtn'].addEventListener('click', e => {
    Model.load();
}, true);